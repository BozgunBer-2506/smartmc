...
### Fixed
- **Connector reconnect failed forever after a soft delete** (Phase 21.1, `docs/STATUS.md`'s long-standing known follow-up) - two distinct causes. The app-level "already connected" check (all four connectors: Telegram, Discord, Slack, Email) never filtered `deletedAt: null`, fixed via a shared `findActiveLinkedAccount()` helper - though this turned out to already be covered by `withSoftDeletes()`'s global Prisma extension, so it wasn't the actual blocker. The real bug, found via this phase's own regression test actually attempting a reconnect: `linked_accounts`' unique index on `(workspace_id, provider_id, external_account_id)` had no soft-delete awareness at the database level, so a soft-deleted row still occupied that key and a genuine reconnect's `INSERT` hit a real Postgres unique-violation (`P2002`) regardless of application-level filtering. Fixed with a partial unique index (`WHERE deleted_at IS NULL`) - Prisma's schema DSL has no partial-index support, so this is a hand-written migration, deliberately unmanaged by `schema.prisma`'s `@@unique`. Verified end to end (7/7, `scripts/verify-phase21.1-reconnect.mjs`): a real soft delete, a real reconnect proven to succeed, both rows confirmed to coexist afterward.
