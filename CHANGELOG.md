...
- **A real Telegram message after a reconnect has never been confirmed reaching the Inbox** - the reconnect/disconnect database behavior (Phase 21.1) is solid and repeatedly verified, but end-to-end message delivery post-reconnect is unconfirmed in every attempt so far. One real contributing factor found: `getWebhookInfo` showed an empty webhook URL after a reconnect, even though the connect response reported `webhookRegistered: true`. Open for a future session.
