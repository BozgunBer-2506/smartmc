...
### Added
- **Phase 22.2 - Design System, remaining screens** - audited `Rules.tsx`, `ConnectorManagement.tsx`, and `AuthForm.tsx` read-only before writing code, confirming Phase 22's color consolidation had reached only `Inbox.tsx`: `ConnectorManagement.tsx` still had the three-way inconsistent success/danger colors and the warning-reuses-priority-hue violation. Migrated in order (`AuthForm` → `ConnectorManagement` → `Rules`), each checkpoint-built independently. `AuthForm.tsx` wrapped in `Card`; `PasswordInput.tsx` moved onto the shared `Input`. `ConnectorManagement.tsx`'s `healthFor()` now returns the shared `status-success`/`status-warning`/`status-danger` tokens, and its disconnect confirmation is now a real Radix `Dialog` (destructive styling, keyboard/focus management) in place of the prior confirmation, same confirm/cancel behavior preserved. `Rules.tsx` - the largest file, and the last one still on inline styles - is fully migrated onto `Card`/`Badge`/`Button`/`Input`, gives `Select` and `Checkbox` their first real product use (trigger/condition/action pickers, VIP test/override checkboxes), moves the rule-list and execution-history loading states onto `Skeleton`, and consolidates its remaining off-token colors (`#0B0F17`/`#1B2333`) onto `surface-1`/`surface-2`. All five product screens are now off the pre-Phase-22 inline-style pattern. No backend change, no functional change to auth/connector/rule behavior.
- **Phase 22 - Design System foundation + Inbox migration** - audited `packages/ui` (a single placeholder `Button`), every authenticated screen, and `apps/marketing-site`'s already-working Tailwind/Radix/CVA pattern before writing code. Found two real inconsistencies in the pre-existing inline styles: two different greens for `success` and two different reds for `danger` across `Inbox.tsx`/`Rules.tsx`/`ConnectorManagement.tsx`, and `warning` reusing the exact hex of the priority accent - a direct violation of `docs/DESIGN_SYSTEM.md` Section 4.1's rule that the two must be visually distinct hues. New `packages/design-tokens` (color/typography/spacing/radius/shadow/motion, light+dark pairs); Tailwind added to `apps/web` for the first time; real `packages/ui` primitives (Button, Input, Card, Badge, ProviderBadge, Alert, Select, Checkbox, Switch, Dialog, Tooltip, Skeleton) built on Radix UI + `class-variance-authority`, matching the marketing site's proven pattern rather than adopting the shadcn CLI as a second workflow. Full theme infrastructure: system/light/dark with `localStorage` persistence, live system-preference tracking, a no-flash inline script, and a small reachable `ThemeToggle` button - not just plumbing. `Inbox.tsx` is the first screen migrated onto the new system, chosen to prove the primitives against Phase 21.3's real behaviors (provider badges, unread state, loading skeletons, filtered/empty states). `Rules.tsx`/`ConnectorManagement.tsx`/`AuthForm.tsx` migration is deliberately deferred to Phase 22.2.
- **Phase 21.5 - Demo/Onboarding** - audited workspace creation, the connector connect flow, empty-inbox behavior, and the demo-data mechanism before writing code (registration already auto-provisions a starter rule and AI credits; Connector Management already covers "connect a first account" - neither needed changes). Fixed two real production gaps instead: the dev-only "Send mock message" control was unconditionally visible in production despite its backend endpoint 404ing there by design - now gated behind `NODE_ENV !== "production"`, matching the backend's own guard; and the empty-inbox message told new users to use that broken tool - replaced with a real "connect your first account" message and a button into the existing Connector Management screen. A production-safe demo-data-seeding mechanism is explicitly deferred as a separate future feature (real open design questions - which sample data, distinguishing demo from real messages, dedup, deletability - make it bigger than this pass). Zero new backend endpoints. Closes out Phase 21 (Product Hardening & UX) in full.
- **Phase 21.4 - Automation UX** - audited the existing `RulesController`/`Rule`/`RuleExecutionLog` backend and `Rules.tsx` frontend before writing any code (per explicit user direction, to avoid a "documented but not built" gap); found the backend already essentially complete, so this phase adds **zero new backend endpoints**. Execution-history entries now show `errorDetail` and each action's `output`/`error` (previously silently dropped even though the API already returned them); a "Edit" action on each rule loads its trigger/conditions/actions back into the existing form for real editing via the existing `PATCH .../:id` + `If-Match` path (previously only `isEnabled` was ever editable from the UI); the execution-history panel's silent `catch` is replaced with a visible loading state and a retryable error. Nested condition-group UI, rule templates, retry/circuit-breaker/DLQ, the simulator/debugger, analytics, and import/export remain explicitly out of scope, per `docs/reviews/phase-10-review.md`'s own prior disclosure.
- **Phase 21.3 - Inbox UX** - scoped to five concrete gaps found by direct testing rather than `UI_GUIDE.md`'s full Inbox vision: a small per-provider badge on each conversation row (`c.providerKey`), a real bold/dot/border unread-vs-read visual distinction (previously both states looked nearly identical), loading-state placeholders for the conversation list and message pane (previously the "None yet" empty message could flash before the first fetch even resolved - a real bug), a distinct "no conversations match your filters" empty state with a one-click "Clear filters" action, and manual mark-unread. New `POST /v1/conversations/:id/unread` (`apps/api/src/conversations/conversations.controller.ts`) mirrors the existing `markRead` handler exactly (`lastReadAt: null`); `apps/web/lib/api.ts` gained `markConversationUnread()`. Explicitly out of scope, disclosed: cursor pagination, a real per-conversation unread *count* (Prisma can't express the required correlated filter in one query without raw SQL; an approximate count would violate `UI_GUIDE.md`'s "the count is a promise" principle), and the Design System. `scripts/verify-phase9.mjs` extended with the new endpoint's checks (25/25).
- **Phase 21.6 - Scheduled message send** - `POST /v1/conversations/:id/messages` accepts an optional `sendAt`; a future timestamp creates a durable `ScheduledMessage` row plus a matching BullMQ delayed job instead of sending immediately, returning `202 Accepted`. A new `ScheduledMessageService` mirrors `SchedulerService`'s existing durable-row + delayed-job pattern (`time.no_reply_after`'s own mechanism), with its own table and BullMQ queue rather than reusing the rule-coupled `ScheduledJob`. The outbound-send logic was extracted from `ConversationsController.sendMessage` into a shared `MessageSendService`, so a fired scheduled message and a live reply go through the identical connector/credential/persist/audit path. New `GET /v1/scheduled-messages` (list) and `POST /v1/scheduled-messages/:id/cancel`. `apps/web/components/Inbox.tsx` gained a "Schedule" toggle on the reply box (datetime picker) and a pending-scheduled-messages list with cancel. Verified locally (`scripts/verify-phase21.6-scheduled-messages.mjs`, 18/18) plus a manual check confirming the delayed job actually fires at its scheduled time; real-provider delivery in production is not yet verified (disclosed gap, see `docs/STATUS.md`).
- **Phase 20.1 - Real rate limiting** (`API.md` Section 9) - a global Redis-backed `RateLimitGuard`, tiered by `Organization.planTier`, with a separate tighter budget for `/v1/ai/*`. `X-RateLimit-Limit`/`Remaining`/`Reset` headers on every request, `429` RFC 7807 with `Retry-After` once exceeded. Live-verified in production: correct headers and 429s at the documented free-tier limits (60/min general, 30/min AI), independent AI/general buckets, real Redis counters, and an 80-concurrent-request load test landing exactly 60/60 - the Redis `INCR` counter is race-free under real concurrency. `docs/ROADMAP.md`'s Phase 20 (Production Readiness) is being delivered as independent sub-phases (20.1-20.5), each with its own commit and live verification, rather than one large effort.
- **Phase 20.5 - Observability foundation** (`docs/ARCHITECTURE.md` Section 7) - real request correlation IDs (`X-Trace-Id` response header, honored if client-supplied, shared between a request's structured access log and its RFC 7807 error body's `traceId` - closing a prior inconsistency where the error path minted its own unrelated ID), one structured JSON log line per HTTP request (`pino`), and a real Prometheus-exposition `GET /metrics` endpoint (`prom-client`): default Node.js process metrics plus `http_requests_total`/`http_request_duration_seconds`/`http_errors_total` (by route pattern, not raw path, to avoid unbounded cardinality), `connector_messages_received_total{provider}`, `automation_rule_executions_total{status}`, and `bullmq_jobs_processed_total`/`bullmq_jobs_failed_total{queue}` for both BullMQ queues. Every metric verified to actually move under real traffic. Deliberately scoped down from the full target stack: OpenTelemetry tracing and a deployed Prometheus/Grafana/Loki stack are not built this round - no real infrastructure was stood up for them, disclosed rather than claimed. Verified end to end, locally and in production (`scripts/verify-phase20.5-observability.mjs`: 15/15 locally, 13/13 in production - the mock-connector-dependent increment check correctly SKIPs there, matching prior Phase 20.x scripts).
- **Phase 20.4 - Real `ETag`/`If-Match` optimistic concurrency** (`API.md` Section 8) - `GET /v1/rules/:id` and `GET/PATCH /v1/notification-preferences` expose a real `ETag` (the row's `version`); `PATCH` requires `If-Match` (`428` if absent, `412 Precondition Failed` with `code: OPTIMISTIC_LOCK_FAILURE` on a stale one); `GET` honors `If-None-Match` for a bodyless `304`. The atomic DB check is against the client's declared `If-Match`, not a version the server just re-fetched in the same request. `NotificationPreference` gained a `version` column (previously had none, and `PATCH` was a blind `upsert` with zero conflict detection) - a first-ever save uses `If-Match: "new"`. `apps/web` wired to send real `If-Match`. Scope: `Rule` and `NotificationPreference` only - the two resources with both a `version` column and an existing mutation endpoint; `LinkedAccount` connector settings and `Workspace`/`Organization` settings are deferred (no settings-mutation endpoint / no version column yet). Verified end to end, locally and in production (25/25, `scripts/verify-phase20.4-etag-concurrency.mjs`), including a genuine two-client race (both fetch, one writes, the other's stale `If-Match` 412s instead of silently overwriting).
- **Phase 20.3 - `?sortBy=`/`?order=` support** (`API.md` Section 4) - a real per-resource sort-field whitelist on `GET /v1/conversations` (`lastMessageAt`/`createdAt`), `/v1/contacts` (`displayName`/`createdAt`), and `/v1/rules` (`createdAt`/`updatedAt`/`name`), plus `?order=` on `/v1/notifications` and `/v1/ai/credits/ledger` (each has exactly one sortable timestamp). An unrecognized `sortBy` falls back to the resource's default rather than erroring. `conversations`/`rules` keep their existing compound default ranking when no `sortBy` is given - an explicit `sortBy` opts out of it, not layers on top. Cursors are now self-describing (carry the `sortBy`/`order` they were minted under via `cursor-pagination.ts`'s new `keysetOr`/`parseSortBy`/`parseOrder`), so a multi-page walk stays correct even if a client stops resending the query params after page 1 - verified directly. Found and closed along the way: `Contact`, `Conversation`, and `Notification` had no database index at all, so every list query on them (including Phase 20.2's cursor pagination) was a full table scan - see `packages/database/prisma/migrations/20260805000000_phase20_3_sort_indexes/`. Verified end to end, locally and in production (`scripts/verify-phase20.3-sort-order.mjs`: 17/17 locally, 15/15 in production - the 2 conversation-sortBy checks correctly SKIP there, since the Mock Connector they seed data through is intentionally 404'd outside development).
- **Phase 20.2 - Real cursor pagination** (`API.md` Section 4) - a shared `apps/api/src/common/cursor-pagination.ts` utility (`parseLimit`/`encodeCursor`/`decodeCursor`/`buildPage`) backs one canonical `{data, pagination: {nextCursor, hasMore}}` envelope, migrated across all 11 list endpoints that needed it (`GET /v1/notifications`, `/v1/rules`, `/v1/rules/:id/executions`, `/v1/ai/credits/ledger`, `/v1/contacts`, `/v1/identity/merge-suggestions`, `/v1/conversations`, `/v1/conversations/:id/messages`, `/v1/search/messages`, `/v1/search/contacts`, `/v1/search`). Real Postgres keyset pagination throughout (`WHERE (sortKey, id) < (lastSortKey, lastId)`, never `OFFSET`), with a stable deterministic sort per resource - including a `ts_rank`-keyed CTE for full-text search results and a `(priorityScore, lastMessageAt, id)` key with null-handling for the main conversation list. Three fully-unbounded queries found during the migration audit (`GET /v1/rules`, `/v1/identity/merge-suggestions`, `/v1/conversations/:id/messages`) are now bounded as part of this phase. `apps/web/lib/api.ts` and every affected `scripts/verify-*` script were swept to unwrap the new envelope. Verified end to end, locally and in production (14/14, `scripts/verify-phase20.2-cursor-pagination.mjs`): full multi-page cursor walk, no duplicate/skipped rows, stable ordering, correct `hasMore`/`nextCursor` on the last page, malformed-cursor safety, and limit clamping.
...
### Fixed
- **Every `packages/ui` primitive's keyboard focus ring was invisible** - found via a post-Phase-22.2 code-level accessibility audit. `globals.css` defines a correct global `:focus-visible` rule, but `Button`/`Input`/`Checkbox`/`Select`/`Switch`/`Dialog`/`ThemeToggle` all also carried Tailwind's `focus-visible:outline-none` utility, which compiles to `outline: 2px solid transparent` (not `outline: none`) - a class-based selector that always beats the bare `:focus-visible` rule on CSS specificity, regardless of source order. Fixed by removing the redundant utility from all seven primitives rather than duplicating a focus-ring definition locally. Affected every screen migrated in Phase 22/22.2. Not yet confirmed with a real Tab-key pass in a browser (no browser-automation tool available) - fix is CSS-specificity-provable, not yet eyes-on-verified.
- **A conversation's `linkedAccountId` never followed a reconnect, permanently blocking outbound send** - found via a real production Phase 21.6 scheduled-message test that fired correctly (BullMQ, `ScheduledMessageService`) but failed at `MessageSendService`'s `LINKED_ACCOUNT_REAUTH_REQUIRED` guard before ever reaching Telegram. Root cause: `EventsProcessor`'s conversation `upsert` only set `linkedAccountId` on `create`, never on `update` - so a conversation that outlives a disconnect/reconnect (which always creates a new `LinkedAccount` row, Phase 21.1's own design) stays pointed at the old, soft-deleted account forever. Pre-existing, not introduced by Phase 21.6 - any plain synchronous reply through an affected conversation would have failed identically. Fixed by re-setting `linkedAccountId` on every inbound message that carries one. Two already-affected production conversations repaired directly via `psql`. `scripts/verify-conversation-relink.mjs` (5/5). A same-day retest confirmed a real scheduled Telegram message now delivers end to end, both at the API level and visually confirmed in the real Telegram client.
- **Connector Management UI** (Phase 21.2, expanded) - the real frontend gap behind "why is there no disconnect button": a UI for connecting/disconnecting connectors was specified from the start (`docs/UI_GUIDE.md` Section 20/24) and explicitly deferred at Telegram's own launch (`docs/reviews/phase-4-sprint-2-review.md`), then never picked back up across seven subsequent phases - not a newly-discovered bug, a long-standing documented gap finally closed. A new `apps/web/components/ConnectorManagement.tsx` screen (reached via a "Connectors" nav button) lists every connected account with a derived health label, a disconnect button behind a confirmation step naming what's retained, and the four providers' connect forms - moved wholesale out of `Inbox.tsx` (they already existed, working, since Phase 4-8), not rebuilt. Verified with a real click-through in a real browser, locally and in production: connect (a real, freshly-rotated Telegram bot token), the entry appearing with a live health label, disconnect with its confirmation dialog, and reconnect - and, for the first time this project has managed to confirm it, a real Telegram message sent after reconnecting through the UI, observed landing in the Inbox, closing Phase 21.1's own long-open item in the same pass.
- **`degraded`→`disconnecting` lifecycle transition was illegal** - found live disconnecting a real production Telegram connector stuck in `degraded` status (an `Unauthorized` polling error), while click-testing the new Connector Management UI. `packages/connector-sdk/src/lifecycle.ts`'s transition table omitted `disconnecting` from `degraded`'s allowed exits, even though `docs/CONNECTOR_SDK.md`'s own state table already listed `degraded` as a valid source for `disconnecting` - a real inconsistency within the doc, faithfully (and therefore incorrectly) implemented in code. Meant a connector stuck `degraded` - exactly when a user most needs to disconnect it - never could be. Fixed in both code and doc.
- **`GET /v1/connectors` - connector management visibility** (Phase 21.2) - the real gap found while fixing Phase 21.1: a workspace's connected connectors had no listing endpoint at all, only a direct database query. One endpoint, not cursor-paginated (same reasoning as `GET /v1/auth/sessions`), returning provider, status, externalAccountId, lastSyncedAt, and a sanitized `lastError` for every connector in the caller's workspace. `LinkedAccount.lastError` is written verbatim by each connector's pre-existing reconciliation service; a new `sanitizeErrorMessage()` helper (`apps/api/src/common/sanitize-error.ts`) strips anything token/URL-query-shaped before it ever reaches a response. A real per-provider network health check (Telegram `getWebhookInfo`, Slack `auth.test`, IMAP `NOOP`) was deliberately deferred, not built speculatively. Verified end to end, locally and in production (13/13 both, `scripts/verify-phase21.2-connectors.mjs`): unauthenticated rejection, empty-list correctness, a real connected Telegram account appearing with the right shape, confirmed the raw bot token never appears anywhere in the response, workspace isolation, and disconnect removing an entry from the live list.
- **Connector reconnect failed forever after a soft delete** (Phase 21.1, `docs/STATUS.md`'s long-standing known follow-up) - two distinct causes. The app-level "already connected" check (all four connectors: Telegram, Discord, Slack, Email) never filtered `deletedAt: null`, fixed via a shared `findActiveLinkedAccount()` helper - though this turned out to already be covered by `withSoftDeletes()`'s global Prisma extension, so it wasn't the actual blocker. The real bug, found via this phase's own regression test actually attempting a reconnect: `linked_accounts`' unique index on `(workspace_id, provider_id, external_account_id)` had no soft-delete awareness at the database level, so a soft-deleted row still occupied that key and a genuine reconnect's `INSERT` hit a real Postgres unique-violation (`P2002`) regardless of application-level filtering. Fixed with a partial unique index (`WHERE deleted_at IS NULL`) - Prisma's schema DSL has no partial-index support, so this is a hand-written migration, deliberately unmanaged by `schema.prisma`'s `@@unique`. The database-level fix is verified repeatedly against production with a real Telegram bot (connect → disconnect → reconnect, 201, no `P2002`); end-to-end message delivery to the Inbox after a reconnect remains unconfirmed (see Known Issues below) - an earlier version of this entry overstated that as verified, corrected 2026-08-13. `scripts/verify-phase21.1-reconnect.mjs` (7/7 locally, direct Prisma-level regression test), `scripts/verify-phase21.1-production-live.mjs` (11-12/12 against production, pure HTTP, no direct database access, across several runs).
- **`PATCH /v1/rules/:id` never actually enforced client-side optimistic concurrency** - the old code compared against a version it had just re-fetched itself moments earlier in the same request, not the version the client actually declared, so the real "someone else edited this since I loaded the form" conflict could never fire. Found and fixed via Phase 20.4's own `If-Match` work.
- **A priority-only `PATCH /v1/rules/:id` silently kept the old priority** - the "simple edit" shortcut branch special-cased `isEnabled` but not `priority`, so `{ priority: 5 }` alone was a no-op. Found via Phase 20.4's own conflict test.
- **`app.setGlobalPrefix`'s `health` exclude only matched the exact `/health` path**, not sub-routes under it - harmless until a temporary Phase 20.4 diagnostic route landed at `/v1/health/...` instead of `/health/...`. Fixed to `["health", "health/(.*)", "dev/(.*)"]`.
- **`turbo.json` had no `concurrency` override**, defaulting to 10 while the workspace has 11 persistent `dev` tasks - `pnpm dev` refused to start. Set `"concurrency": "20"`. Also switched `"ui"` from `"tui"` to `"stream"`, since the TUI pane mode was separately observed misreporting healthy persistent tasks as failed immediately after they reached their steady "watching" state.
...
- **`apps/web`/`apps/marketing-site` unreachable in production (`502`)** - both hardcoded `next start -p <fixed port>`, ignoring Railway's dynamically-assigned `PORT`. Fixed to `next start -p ${PORT:-<default>}`.
- **`apps/web` calling `localhost:4000` from real visitors' browsers** - `NEXT_PUBLIC_API_URL` was never set on Railway; since it's build-time-inlined by Next.js, the shipped bundle carried the code's dev fallback. Fixed by setting the variable and rebuilding; confirmed present in the actual served JS bundle.
- **Session lost on every page reload in production** - the refresh-token cookie was `SameSite=Strict`, but `apps/web`/`apps/api` are on different sites per the Public Suffix List (`up.railway.app` subdomains), so the cookie was silently dropped on `tryRefresh()`'s cross-site fetch. Fixed: `SameSite=None` in production, now configurable via `AUTH_COOKIE_SAMESITE` (`apps/api/src/config/auth.config.ts`).
- **Slack connect `500 missing_scope`** - the default bot OAuth scope list never requested `groups:read`, needed for `conversations.list`'s `private_channel` half despite `groups:history` already being requested for reading them. Added `groups:read` to the default scope list (`apps/api/src/config/slack.config.ts`).
- **Slack connect `500 not_in_channel`** - `syncChannels()` called `conversations.history` on every channel `conversations.list` returned, including channels the bot was never invited to. Fixed by filtering to `is_member: true` channels before building the sync cursor (`packages/connector-sdk/src/slack/slack-connector.ts`).
- **A pushed, committed bug fix never actually deployed** - `@smc/api`'s Railway Watch Paths setting (`/apps/api/**`) didn't cover `packages/*` dependencies, so changes there were silently skipped by Railway's build system across three separate deploy attempts. Fixed by adding `/packages/**` to Watch Paths - Railway dashboard config, not code. See [ADR-0024](docs/adr/0024-railway-watch-paths-must-cover-workspace-dependencies.md).
- **Slack channel-join/leave notices ingested as real Inbox messages** - the Events API webhook filter checked `event.bot_id` but never `event.subtype`, so Slack's own "X joined the channel" notices (which carry `type: "message"` too) were stored and shown as if a user had sent them. Fixed with a shared `isSlackContentMessage()` helper (`packages/connector-sdk`) used by both the webhook path and `initialSync`, narrow enough to still pass through real content like `file_share` attachment placeholders. Also fixed a certification-suite regression this uncovered - `certify-slack-connector.mjs`'s fake channel fixture never set `is_member`, so the earlier `not_in_channel` fix had been silently degrading certification to 13/3 since it landed.
- **Production was encrypting every connector credential with a public, hardcoded key** - `CREDENTIALS_ENCRYPTION_KEY` was never set on Railway, so `credentials-store.config.ts` silently fell back to a key derived from a literal string committed in this public repo. Fixed: generated a real random key, set it on Railway, and rotated affected connections (only test data existed, verified first, so old records were deleted and reconnected fresh rather than migrated).
- **Railway build failure** (`error TS2742`, `apps/api/src/common/linked-account.ts`) - `findActiveLinkedAccount`'s inferred return type couldn't be portably named by `tsc`'s declaration-emit. Never caught locally because `pnpm dev` runs via `ts-node`, which doesn't perform declaration-emit checks the way a real `tsc -p tsconfig.json` build does - a real local-vs-CI parity gap. Fixed with an explicit `Promise<LinkedAccount | null>` return type; confirmed with a real `pnpm --filter @smc/api build` run before pushing.

### Security
- **Secret rotation** (2026-07-30/31) - Telegram bot token, Slack bot token/Client Secret/Signing Secret, and the production Postgres password were all rotated after being exposed in this session's chat history (or, for the encryption key, after the finding above). Each rotation was verified live: OAuth re-authorized, Events API signature re-verified with a real HMAC, and a full login → database round trip confirmed after the Postgres password change (which required a manual `@smc/api` redeploy to pick up the new connection string - briefly surfaced as `database: "error"` on `/health` until it did).

### Known Issues (found live, not yet fixed - see `docs/STATUS.md`'s Railway Production Deployment section)
- ~~Connector reconnect fails after soft delete~~ - **fixed, Phase 21.1** (see Fixed section above).
- ~~A real Telegram message after a reconnect has never been confirmed reaching the Inbox~~ - **resolved, Phase 21.2** (see Fixed section above) - confirmed end to end via the Connector Management UI once the `degraded`→`disconnecting` lifecycle bug blocking it was fixed.
- **`prestart`'s automatic `prisma migrate deploy` silently skipped a migration once** (Phase 21.1's partial-unique-index migration) - worked around via direct `psql`, not root-caused. Every migration before and since has applied automatically without issue.
- **Email connector can't complete `connect` in production - outbound SMTP is platform-blocked** - live-verified: IMAP (receive) connects and authenticates in under 2 seconds with a real Gmail account; SMTP (send, port 587) genuinely times out (`ETIMEDOUT`) reaching Gmail, matching cloud/PaaS providers' common default block on outbound SMTP submission ports for anti-spam reasons - not a credentials, TLS, or code bug (all independently ruled out). `EmailApiClient.testConnection()` requires both to succeed, so the connector can't connect at all right now despite receive working perfectly. Deliberately deferred - see `docs/STATUS.md` for the options considered (request Railway lift the block, move sending to an HTTPS email API, or allow IMAP-only connections).
