...
### Fixed
- **Connector reconnect failed forever after a soft delete** (Phase 21.1, `docs/STATUS.md`'s long-standing known follow-up) - two distinct causes. The app-level "already connected" check (all four connectors: Telegram, Discord, Slack, Email) never filtered `deletedAt: null`, fixed via a shared `findActiveLinkedAccount()` helper - though this turned out to already be covered by `withSoftDeletes()`'s global Prisma extension, so it wasn't the actual blocker. The real bug, found via this phase's own regression test actually attempting a reconnect: `linked_accounts`' unique index on `(workspace_id, provider_id, external_account_id)` had no soft-delete awareness at the database level, so a soft-deleted row still occupied that key and a genuine reconnect's `INSERT` hit a real Postgres unique-violation (`P2002`) regardless of application-level filtering. Fixed with a partial unique index (`WHERE deleted_at IS NULL`) - Prisma's schema DSL has no partial-index support, so this is a hand-written migration, deliberately unmanaged by `schema.prisma`'s `@@unique`. Verified end to end, locally and in production: 7/7 locally (`scripts/verify-phase21.1-reconnect.mjs`, direct Prisma-level regression test), 12/12 in production (`scripts/verify-phase21.1-production-live.mjs`, pure HTTP, no direct database access) - a real Telegram bot connect → disconnect → reconnect (201, no `P2002`) → a real message sent live during the run, confirmed landing in the Inbox. Production rollout also surfaced two disclosed findings: the `prestart` auto-migration hook silently skipped this phase's migration on first deploy (worked around via direct `psql`, not root-caused), and `TelegramController.disconnect()` clears Telegram's bot-token-global webhook regardless of which workspace's connection triggered it (inherent to Telegram's Bot API, not a regression) - see `docs/STATUS.md`.
...
- **Connector reconnect fails after soft delete** - `TelegramController`/`DiscordController`/`SlackController`'s "already connected" check never filters `deletedAt: null`, so any `LinkedAccount` row (even a disconnected or failed-attempt one) permanently blocks reconnecting the same external account.
