...
- [x] **Phase 21.1 - Soft-delete reconnect bug** - complete 2026-08-09, live-verified in production 2026-08-10/11 with a real Telegram bot end to end. Closes a real, long-standing bug first surfaced during Phase 20's Slack live verification and the encryption-key rotation (docs/STATUS.md's "Known follow-up," open since those incidents): once any `LinkedAccount` row existed for a given external account, that same account could never be reconnected - every future connect attempt reported "already connected" forever, even after a genuine disconnect. Two distinct causes, both fixed:
  ...
  See `scripts/verify-phase21.1-reconnect.mjs` (7/7 locally): a real soft-delete via Prisma, a real reconnect `create()` proven to succeed (not caught-and-ignored), and confirmation that both the old (soft-deleted) and new (active) rows coexist in the table - a true soft delete, not a disguised hard delete.
