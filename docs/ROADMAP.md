...
- [x] **Phase 21.1 - Soft-delete reconnect bug** - complete 2026-08-09, the core database-level fix live-verified in production repeatedly (2026-08-10/11) with a real Telegram bot. Closes a real, long-standing bug first surfaced during Phase 20's Slack live verification and the encryption-key rotation (docs/STATUS.md's "Known follow-up," open since those incidents): once any `LinkedAccount` row existed for a given external account, that same account could never be reconnected - every future connect attempt reported "already connected" forever, even after a genuine disconnect. Two distinct causes, both fixed:
  ...
  **Two things found along the way, disclosed rather than smoothed over:**
  ...
  - ~~The end-to-end "does a real Telegram message reach the Inbox after reconnect" check has never actually passed~~ - **resolved 2026-08-15**, confirmed via a real click-through in production (see Phase 21.2 below): disconnect → reconnect through the real Connector Management UI → a real message sent from a real Telegram client → observed landing in the Inbox. The earlier empty-`getWebhookInfo`-URL finding (this same session, an earlier attempt) was real; whether it shared a root cause with the runs that failed before this one, or was a separate transient issue, was not conclusively isolated - what's confirmed is that the flow works end to end now.
  ...
  **Production deployment surfaced two more real findings, both resolved live:**
  ...
  The reconnect/disconnect database lifecycle is live-verified end to end via `scripts/verify-phase21.1-production-live.mjs` (pure HTTP, no direct database access, run with a real `TELEGRAM_TEST_BOT_TOKEN` against `https://smcapi-production-bc04.up.railway.app`, 11-12/12 depending on the run): real active LinkedAccount → real disconnect → real reconnect (**201, no `P2002`** - the actual regression this phase fixes) → confirmed as a genuinely new row, not the old one → old row confirmed gone from the active set. That script's *message-delivery* check (it asks a human to send a real Telegram message during a 30-second polling window) never once passed automated - it depends on a human being present at exactly the right moment, which none of this script's runs had. The end-to-end message delivery claim is instead backed by the real, manual UI click-through recorded under Phase 21.2 below, which is what actually confirmed it for the first time, 2026-08-15.
- [x] **Phase 21.2 - Connector management visibility** - complete 2026-08-13, live-verified in production, deliberately scoped to a single endpoint per explicit user direction (a separate real-network `/health` endpoint per connector - Telegram `getWebhookInfo`, Slack `auth.test`, IMAP `NOOP` - deferred until an actual need for it, not built speculatively). `GET /v1/connectors` closes the real gap found while fixing Phase 21.1: a workspace's connected connectors had no listing endpoint at all, only a direct database query. Not cursor-paginated - same reasoning `GET /v1/auth/sessions` already established (a workspace's own connector list is small and user-owned, disproportionate to paginate). `LinkedAccount.lastError` (written verbatim by each connector's pre-existing reconciliation service - untouched, out of this phase's scope) is sanitized at the response boundary via a new `sanitizeErrorMessage()` helper (`apps/api/src/common/sanitize-error.ts`) before ever reaching a client - strips anything token/URL-query-shaped, truncates, never passes through unredacted. See `scripts/verify-phase21.2-connectors.mjs`: 13/13 locally and 13/13 in production - unauthenticated rejection, empty-list correctness, a real connected Telegram account appearing with the right shape, confirmed the raw bot token never appears anywhere in the response JSON, workspace isolation (a second user never sees the first user's connector), and disconnect correctly removing an entry from the live list.
  ...
  **Expanded 2026-08-13/15, per explicit user direction, to close a real frontend gap: there was no UI to connect or disconnect a connector at all.** This was not a newly-discovered bug - `docs/UI_GUIDE.md` Section 20/24 specified a real Connector Management screen (list, status/health, a disconnect confirmation step naming what's retained) from the start, and `docs/reviews/phase-4-sprint-2-review.md`'s Deliberate Simplifications table #4 explicitly deferred building it at Telegram's own launch ("the raw data is already captured and available for whenever that screen is built") - it just never got picked back up across seven subsequent phases. Built: a new `apps/web/components/ConnectorManagement.tsx` screen (reached via a new "Connectors" nav button in `Inbox.tsx`, same `view`-state pattern `Rules.tsx` already established) rendering `GET /v1/connectors`' list with a derived health label per `CONNECTOR_SDK.md` Section 2 status, a disconnect button behind a confirmation step, and the four providers' connect forms (Telegram bot-token input, Discord/Slack OAuth-redirect buttons, Email IMAP/SMTP form) - these forms already existed inline in `Inbox.tsx` since Phase 4-8 and were moved here wholesale, not rebuilt. `apps/web/lib/api.ts` gained `fetchConnectors`/`disconnectConnector`. Verified with a real click-through in a real browser, both locally and in production: connect (Telegram, with a freshly-rotated real bot token), the connection appearing in the list with a live health label, disconnect with its confirmation dialog, and reconnect - all working end to end.

  **A real bug found via this exact production click-through, fixed same-day**: disconnecting a connector currently in `degraded` status (a real production Telegram connection with an `Unauthorized` polling error) threw `IllegalLifecycleTransitionError` - `degraded`→`disconnecting` was missing from `packages/connector-sdk/src/lifecycle.ts`'s transition table, even though `docs/CONNECTOR_SDK.md`'s own state table already listed `degraded` as a valid "Entered from" source for `disconnecting` - a real inconsistency within the doc itself, faithfully (and therefore incorrectly) implemented in code. This meant a connector stuck in `degraded` - exactly the state a user most needs to escape - could never be disconnected through the normal flow. Fixed in both the code and the doc table; the other three `disconnecting`-reachable states (`active`, `reauth_required`, `error`) were already consistent. After the fix, the full real-world sequence was confirmed live: disconnect the degraded connector → reconnect with a real bot token → send a real Telegram message → observe it land in the Inbox - closing Phase 21.1's own long-open "message delivery after reconnect" item in the same pass (see above).
- [x] **Phase 21.3 - Inbox UX** - complete 2026-08-17, deliberately scoped down from this section's original full description (search/filter already existed since Phase 9) to five concrete gaps found by direct user testing: provider indicators, a real unread/read visual distinction, loading states, empty-state ambiguity, and manual mark-unread. Explicitly excluded per user direction, to keep this pass tight: cursor pagination/"load more" (a separate feature), a real per-conversation unread *count* (see below), and `UI_GUIDE.md`'s full aspirational vision (three-pane layout, Morning Briefing home screen, IdentityGraph avatars, Needs-You-as-default-filter) - that's Design System/later-phase territory, not this pass.
  - **Provider indicator** - a small colored text badge per `c.providerKey` (Telegram/Discord/Slack/Email/Mock), same inline-style pattern as the existing `needsYouBadgeStyle` constant; no icon assets exist yet (Design System scope).
  - **Unread visual distinction** - title is bold only when `c.unread` is true (previously always bold, so read and unread rows looked nearly identical - the real complaint this closes); a colored dot plus a left-border/background tint on the row, replacing the bare `●` character.
  - **Loading states** - new `conversationsLoading`/`messagesLoading` state, gating a simple static skeleton-block placeholder (not a spinner, per `UI_GUIDE.md` Section 17's "skeleton for predictable-shape content" guidance) instead of the previous behavior, which flashed the "None yet" empty message before the first fetch had even resolved - a real bug, not just missing polish.
  - **Empty-state distinction** - "genuinely zero conversations" (existing message, unchanged) is now separate from "your filters matched zero" (a new message plus a one-click "Clear filters" action resetting all four filter states).
  - **Manual mark-unread** - new `POST /v1/conversations/:id/unread` (`apps/api/src/conversations/conversations.controller.ts`), a direct mirror of the existing `markRead` handler (`lastReadAt: null` instead of `new Date()` - `isUnread()` already only ever compares these two timestamps, so no new schema/state was needed). `apps/web/lib/api.ts` gained `markConversationUnread()`; a "Mark unread" button appears on any currently-read conversation row.
  - **Explicitly deferred, disclosed rather than silently built**: a real per-conversation unread *message count* was scoped out during planning - correctly computing it requires comparing each row's own `lastReadAt` against its messages, which Prisma's typed `_count` filter can't express in one query (would need a raw correlated SQL query or an N+1 per-row count), and `UI_GUIDE.md` Section 2's own principle ("the count is a promise, never guessed") argues against shipping an approximate one. Boolean unread with strong visual weight ships instead.
  - Verified: `pnpm --filter @smc/api build` and `pnpm --filter @smc/web build` both clean. `scripts/verify-phase9.mjs` extended with the new endpoint's checks (POST succeeds, flips the flag back, 404 on a nonexistent conversation) - 25/25, no regressions. Full local regression suite (16 scripts, every Phase 1-21.6 verify script) re-run clean. **Not verified this session**: a real browser click-through of the new visual states - no browser-automation tool is available in this environment, the same standing, disclosed limitation every prior UI-touching phase (10/11/12/13/14) has recorded rather than silently skipped.
- [x] **Phase 21.4 - Automation UX** - complete 2026-08-17. Scoped by an explicit audit first (per user direction, to avoid this project's past "documented but not built" trap): `RulesController`'s CRUD, `Rule`/`RuleExecutionLog` schema, and `Rules.tsx` were read end to end before any code was written. Finding: the backend was already essentially complete (full CRUD with `If-Match` optimistic concurrency, cursor-paginated execution logs with real `status`/`errorDetail`/per-action `actionsExecuted`) - the actual gaps were narrowly frontend, so **no new backend endpoint was added this phase**, confirming the audit's own conclusion before writing a line of code.
  - **Execution detail** - the execution-history panel (`Rules.tsx`) previously rendered only `status` and a bare list of action *types*, silently dropping `errorDetail` and each action's `output`/`error` even though the API already returned them. Now shows `errorDetail` on any non-`success` execution, and each action's own status/output/error inline.
  - **Rule edit** - previously the only way to change a rule's trigger/conditions/actions was delete-and-recreate; only `isEnabled` was ever PATCHed from the UI. `handleStartEdit()` now loads an existing rule's full state into the same New Rule form (relabeled "Edit rule" / "Save changes" while active), and submits through the existing `PATCH .../:id` + `If-Match` path (`updateRule()`, unchanged) - no new backend contract, exactly as the audit predicted.
  - **Execution history loading/error state** - the previous silent `catch { /* non-fatal */ }` around `fetchRuleExecutions` is gone; a failed fetch now shows a visible error with Retry, and a brief loading placeholder replaces the instant (and previously misleading) "No executions yet." on first expand.
  - **Explicitly out of scope**, per the audit and confirmed already-disclosed in `docs/reviews/phase-10-review.md`: nested condition-group UI, rule templates/marketplace, retry/circuit-breaker/DLQ, the rule simulator/step-by-step debugger, analytics, import/export. None of these are new findings - all were already deferred at Phase 10, not reopened here.
  - Verified: `pnpm --filter @smc/web build` clean (no backend touched, so no API build needed this time). Full local regression suite re-run - all HTTP-level checks clean; `scripts/verify-phase14.mjs`'s two icon-route failures are a pre-existing, unrelated Next.js dev-server cache corruption (`Cannot find module './chunks/vendor-chunks/...'`, confirmed via the dev server's own log, not present in the actual production build check) - not caused by and not related to this phase's changes. **Not verified this session**: a real browser click-through - same standing, disclosed gap as Phase 21.3 and every UI phase since Phase 10.
- [x] **Phase 21.5 - Demo / onboarding** - complete 2026-08-17, scoped by an audit first (same discipline as 21.3/21.4). Findings: workspace creation already auto-provisions a real starter Rule and AI credit grant on register (Phase 2, unchanged); Connector Management (Phase 21.2) already fully covers "connect a first account" - neither needed touching. Two real, concrete gaps closed:
  - **A dev-only tool was unconditionally visible in production** - the "Send mock message" control (`Inbox.tsx`) called `POST /dev/mock-connector/send`, which the backend itself 404s outside development (`mock-connector.controller.ts`'s own `NODE_ENV === "production"` guard) - so a real production user saw a working-looking button that silently failed. Now gated behind `process.env.NODE_ENV !== "production"`, matching the backend's own guard exactly; unaffected in local dev.
  - **The empty-inbox message told a new user to use that broken tool** - "None yet - send a mock message above" is replaced with a real onboarding message ("No conversations yet" / "Connect your first account to start receiving messages.") plus a "Connect account" button routing straight to the existing Connector Management screen (`onOpenConnectors`, no new prop/route needed). Kept distinct from Phase 21.3's filtered-empty state, not merged with it.
  - **Explicitly deferred, a separate future item, not attempted here**: a production-safe demo-data seeding mechanism (synthetic sample conversations with no real `LinkedAccount`) - real open questions before it could be built responsibly (which sample data, how demo messages are distinguished from real ones, dedup on re-seed, user-deletable, whether it could ever collide with a real inbox) make it a genuinely separate feature, not 21.5-sized polish. `UI_GUIDE.md` Section 21's fuller onboarding vision (forced connect-first flow, a retroactive "you would have missed this" scan, first-rule-from-template wizard, the Morning Briefing home screen) is also out of scope, same as Phase 21.3/21.4's precedent - none of those screens/mechanisms exist yet and building them isn't this pass's job.
  - Verified: `pnpm --filter @smc/web build` clean (no backend touched - zero new endpoints, confirming the audit's own prediction). Full local regression suite re-run, no new failures. **Not verified this session**: a real browser click-through, same disclosed gap as 21.3/21.4.
- [x] **Phase 21.6 - Scheduled message send** - complete 2026-08-15, verified locally and, as of 2026-08-16, live end to end in production including real Telegram delivery (see below). Added per explicit user direction: a reply written outside working hours (the concrete example given: something worth saying at night, sent at 8am instead) had no way to be scheduled - only an immediate, synchronous send existed. `POST /v1/conversations/:id/messages` gained an optional `sendAt`: omitted, or a past/present timestamp, sends immediately exactly as before (`201`); a future timestamp instead creates a durable `ScheduledMessage` row and returns `202 Accepted` - the first real use of `API.md`'s documented-but-previously-unused async response shape for this endpoint. A new `ScheduledMessageService` (`apps/api/src/automation/scheduled-message.service.ts`) mirrors `SchedulerService`'s existing durable-row + BullMQ-delayed-job pattern (`time.no_reply_after`'s own mechanism) rather than reusing `ScheduledJob`, which has a required `ruleId` and no concept of a user-authored message body - its own table (`scheduled_messages`, `packages/database/prisma/migrations/20260815000000_phase21_6_scheduled_messages/`) and its own BullMQ queue (`scheduled-messages`). The actual outbound-send logic was extracted from `ConversationsController.sendMessage` into a shared `MessageSendService` (`apps/api/src/conversations/message-send.service.ts`) so a fired scheduled message goes through the identical connector/credential/persist/realtime/audit-log path a live reply does, not a parallel reimplementation that could drift from it - confirmed directly: a scheduled send that fails (no connected account, provider rejection, etc.) marks the row `failed` with a sanitized `lastError`, never silently drops it. New `GET /v1/scheduled-messages` (list, no pagination - same "small, user-owned list" reasoning as `GET /v1/connectors`) and `POST /v1/scheduled-messages/:id/cancel` (404 if already fired/cancelled/nonexistent, not a silent no-op). `apps/web/components/Inbox.tsx` gained a "Schedule" toggle next to the existing reply box (a `datetime-local` picker + "Confirm schedule" button) and an inline list of the current conversation's pending scheduled messages with a Cancel action, using new `apps/web/lib/api.ts` functions (`scheduleMessage`, `fetchScheduledMessages`, `cancelScheduledMessage`).

  See `scripts/verify-phase21.6-scheduled-messages.mjs` (18/18 locally): auth required on both new endpoints, empty-list correctness, missing-body and unparseable-`sendAt` rejection (400), a future `sendAt` returning `202`/`status: pending` and appearing in the list, a past `sendAt` correctly falling through to the immediate-send path instead of being scheduled, cancel succeeding then 404ing on a second attempt, a nonexistent id 404ing, and workspace isolation (a second user can neither see nor cancel the first user's scheduled message). A separate manual check (not part of the script, run directly against the local BullMQ worker) confirmed the delayed job actually fires at its scheduled time and correctly transitions the row to `failed` with a real, sanitized error when the underlying send fails - the full durable-row-plus-delayed-job mechanism working end to end, not just the HTTP surface around it.

  ~~**Disclosed gap, explicit**: unlike Phase 21.1/21.2, this phase's live-in-production verification does not include an actual real-provider delivery at the scheduled time~~ - **resolved 2026-08-16** (see below): a real production scheduled send was traced through every stage and confirmed landing in the real Telegram client.

  **A real production failure found 2026-08-16, root-caused and fixed same-day**: the first real production scheduled-message test ("This is a msg from smartmc prod test", scheduled 2026-08-15 14:49 local) reached its `sendAt`, fired correctly (BullMQ delayed job, `ScheduledMessageService.fire()`, `MessageSendService.send()` all ran, confirmed directly against production logs and the `scheduled_messages` row), but never reached Telegram - it failed at `MessageSendService`'s own `LINKED_ACCOUNT_REAUTH_REQUIRED` guard, `lastError: "The connected account needs to be reauthorized before sending."`. Root cause, confirmed directly against production data: the conversation's `linked_account_id` pointed at an old, soft-deleted `LinkedAccount` (disconnected the day before); a currently-active `LinkedAccount` for the same bot existed, but nothing had ever re-pointed the conversation to it. `apps/api/src/events/events.processor.ts`'s conversation `upsert` only ever set `linkedAccountId` on `create`, never on `update` - so any conversation surviving a disconnect/reconnect cycle (which always creates a *new* `LinkedAccount` row, per Phase 21.1's own soft-delete design) stayed wired to the dead account forever. This predates Phase 21.6 entirely and would have identically broken a plain synchronous reply - Phase 21.6 didn't introduce it, it's just the first thing that happened to hit it in production. Fixed by re-setting `linkedAccountId` on every inbound message that carries one (`?? undefined` so a payload without one, e.g. the Mock Connector, never nulls out an existing link) - confirmed with a new direct-Prisma regression test, `scripts/verify-conversation-relink.mjs` (5/5): a conversation's `linkedAccountId` now correctly follows a reconnect on the very next inbound message. Two production conversations already affected (the one above, and one other found via the same query pattern) were repaired directly via `psql` - a one-time data fix, not a migration, since the code fix makes this self-healing going forward.

  **Real-provider delivery confirmed end to end, 2026-08-16, closing the disclosed gap above**: a fresh production retest ("SmartMC prod scheduled-send verification - post-fix retest", scheduled ~2.5 minutes out through the same, now-repaired conversation) was traced through every stage - `202` on scheduling, the BullMQ delayed job firing within the same second as its `send_at`, `bullmq_jobs_processed_total{queue="scheduled-messages"}` incrementing (only happens on the worker's own `"completed"` event), the `ScheduledMessage` row transitioning `pending` → `sent` with a `sentMessageId`, and the created `Message.externalId` being a real Telegram `message_id` (`TelegramConnector.send()` only returns one on a successful `sendMessage` API response). The user then confirmed the message actually arrived in the real Telegram client. This is the first time this project has verified a scheduled send's *entire* chain - Inbox → `ScheduledMessageService` → BullMQ/Redis → `MessageSendService` → `TelegramConnector` → Telegram Bot API → real device - with both API-level and human-eyes-on-the-app confirmation.

---

## Phase 22 - Design System / Product UI Overhaul

Added to the roadmap 2026-08-21, per explicit user direction, once Phase 21 (Product Hardening & UX) closed in full: the product's own UI had reached functional parity with its documented requirements, but every screen since Phase 3 was hand-styled with duplicated, drifting inline styles, while `docs/DESIGN_SYSTEM.md` - a complete, "implementation-ready" spec - had sat approved and entirely unbuilt since 2026-07-18. Scoped by an audit first (same discipline as Phase 21.3-21.5): `packages/ui` (a single placeholder `Button`), every authenticated screen, and `apps/marketing-site`'s already-proven Tailwind/Radix/CVA pattern were read before any code was written - see `docs/STATUS.md`'s Phase 22 entry for the full findings, including two real inconsistencies the audit found (two different greens for `success`, two different reds for `danger`, plus `warning` reusing the priority accent's own hex in direct violation of `DESIGN_SYSTEM.md` Section 4.1).

- [x] **Design tokens** - `packages/design-tokens` (new package): color (light+dark pairs for every `DESIGN_SYSTEM.md` Section 4.1 semantic token, dark values kept identical to the product's pre-existing palette so the visual identity doesn't shift, light values newly derived), typography (Section 5's full type scale), spacing/radius/shadow/motion/breakpoints (Section 6-8). Provider badge colors (Section 4.2) live here too. `apps/web/app/globals.css` embeds the generated CSS custom-property block by hand, matching `DESIGN_SYSTEM.md` Section 17's own stated convention that the token source and its CSS variables "stay in lockstep," checked in review rather than build-generated.
- [x] **Tailwind infrastructure** - added to `apps/web` for the first time (`tailwindcss`/`postcss`/`autoprefixer`, `tailwind.config.ts` mapping token names to the CSS variables above). `content` scanning includes `packages/ui/src` so shared primitives' classes aren't purged.
- [x] **UI primitives** (`packages/ui`) - Button (replacing the Phase 1 placeholder), Input, Card, Badge (plus a `ProviderBadge` for the Section 4.2 exception), Alert, Select, Checkbox, Switch, Dialog, Tooltip, Skeleton - Radix UI primitives + `class-variance-authority` variants + Tailwind, the same hand-built pattern `apps/marketing-site` already established (ADR-0020) rather than introducing the shadcn CLI as a second component-generation workflow for no functional difference.
- [x] **Accessibility** - `:focus-visible` is token-driven globally (`globals.css`, the same pattern already proven on the marketing site); Radix supplies correct ARIA roles/keyboard interaction for Dialog/Select/Checkbox/Switch/Tooltip out of the box, which is the main reason Radix was chosen over hand-rolling these from scratch.
- [x] **Theme infrastructure + toggle** - three states (system/light/dark), a manual override persisted to `localStorage`, live system-preference tracking when no override is stored, and a small single-button `ThemeToggle` (cycles system → light → dark) - not a bigger settings panel. No-flash handled by an inline pre-hydration script (`THEME_NO_FLASH_SCRIPT`) that only ever needs to act when a stored override *contradicts* the OS preference - the no-override case is already flash-free on its own, resolved by `@media (prefers-color-scheme: dark)` before any JS runs.
- [x] **First real migration: Inbox** - `apps/web/components/Inbox.tsx` fully migrated off inline styles onto the primitives/tokens above, chosen specifically because Phase 21.3 had just added real, non-trivial behaviors (provider badges, unread/read visual state, loading skeletons, filtered-vs-empty states) that only a real screen migration - not a components-only pass - could actually prove against. Every one of those behaviors is preserved exactly, not rebuilt.
- [x] **Responsive standardization** - the `@media (max-width: 720px)` block duplicated independently across `Inbox.tsx`/`Rules.tsx`/`ConnectorManagement.tsx` is replaced, in the migrated screen, with Tailwind's standard `md:` (768px) breakpoint - `DESIGN_SYSTEM.md` Section 8's own instruction ("standard Tailwind defaults, deliberately not customized"), not a new decision.
- [x] **Documentation** - `docs/DESIGN_SYSTEM.md` updated with an implementation-status note (Sections 4-9 now real, Sections 10+ still spec-only); this entry.

**Explicitly out of scope, deferred to Phase 22.2**: migrating `Rules.tsx`, `ConnectorManagement.tsx`, and `AuthForm.tsx` onto the new system - a deliberate scope split (per explicit user direction) so Phase 22 could answer "does the new design system actually work on a real, already-tested screen" before committing every remaining screen to it in the same pass.

### Phase 22.2 - Design System, remaining screens

- [x] **AuthForm** - wrapped in `Card`, fields moved to `Input`/`PasswordInput` (now itself migrated onto `Input`), errors moved to `Alert variant="danger"` - proves the primitive combination on the product's actual entry point, no redesign.
- [x] **ConnectorManagement** - `healthFor()` rewritten to return semantic Tailwind classes (`text-status-success`/`text-status-warning`/`text-status-danger`), fixing the three real color inconsistencies the Phase 22.2 audit found: two different greens for "success" across screens, and `warning` reusing the priority-accent hue rather than a distinct one. The disconnect confirmation (previously a native `window.confirm`-equivalent) is now a real Radix `Dialog` (destructive-action styling, keyboard/focus management), same confirm/cancel behavior, only the presentation changed.
- [x] **Rules** - fully migrated: `Card`/`Badge`/`Button`/`Input` replace the inline style objects; trigger type, condition field/operator, action type, and AND/OR pickers move from raw `<select>` to `Select`/`SelectTrigger`/`SelectContent`/`SelectItem` (the primitives' first real product use); the VIP test checkbox and silent-hours VIP-override checkbox move to `Checkbox`; the rule list and execution-history loading states use `Skeleton` instead of static "(loading...)" text; the last off-token color literals (`#0B0F17`/`#1B2333`) are now `surface-1`/`surface-2`. No functional change - create/edit/enable/disable/test/history/AI-suggest/notification-preferences all behave exactly as before.
- [x] **Responsive standardization** - `Rules.tsx`'s `@media (max-width: 720px)` block is replaced with Tailwind's `md:` breakpoint, matching Phase 22's Inbox migration.
- [x] **Documentation** - `docs/DESIGN_SYSTEM.md` implementation-status note updated (all five product screens now migrated); this entry.

**Explicitly out of scope**: any new backend endpoint, any change to auth/connector behavior, any functional change to Rules' automation UX, and any large visual redesign beyond adopting the shared primitives/tokens - per explicit user direction, this phase is a migration, not a redesign.

Verified: `pnpm --filter @smc/web build` clean. Full local regression suite (16 scripts) re-run clean after a real, unrelated environment issue (a stale Postgres container port-forwarding state after 2 days of uptime, fixed with a non-destructive `docker restart`, no data loss) was found and resolved mid-session - disclosed rather than glossed over, since it briefly looked like a Phase 22 regression before being root-caused as pre-existing infrastructure drift. **Not verified this session**: a real browser click-through of the migrated Inbox (light/dark toggle, responsive breakpoint, focus states) - no browser-automation tool is available in this environment, the same standing, disclosed gap every UI-touching phase since Phase 10 has recorded.
