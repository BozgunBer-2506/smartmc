...
- [ ] **Phase 20.5 - Observability** (OTel/Prometheus/Grafana/Loki, per `ARCHITECTURE.md`'s Phase 2 - Hardening sketch) - no metrics/tracing/structured log aggregation exists yet beyond NestJS's default console logging. **Deliberately paused, not abandoned, 2026-08-06**: with 20.1-20.4 complete and live-verified, the project judged the marginal return on finishing observability now lower than on presentation work (README, architecture diagrams, demo material, CV/LinkedIn, job applications) - see `docs/STATUS.md`'s Next Action. Fully scoped and ready to resume.
