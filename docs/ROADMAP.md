...
- [x] **Phase 21.6 - Scheduled message send** - complete 2026-08-15, verified locally (production live-verification of an actual scheduled delivery is a disclosed, explicit gap - see below). Added per explicit user direction: a reply written outside working hours (the concrete example given: something worth saying at night, sent at 8am instead) had no way to be scheduled - only an immediate, synchronous send existed. `POST /v1/conversations/:id/messages` gained an optional `sendAt`: omitted, or a past/present timestamp, sends immediately exactly as before (`201`); a future timestamp instead creates a durable `ScheduledMessage` row and returns `202 Accepted` - the first real use of `API.md`'s documented-but-previously-unused async response shape for this endpoint. A new `ScheduledMessageService` (`apps/api/src/automation/scheduled-message.service.ts`) mirrors `SchedulerService`'s existing durable-row + BullMQ-delayed-job pattern (`time.no_reply_after`'s own mechanism) rather than reusing `ScheduledJob`, which has a required `ruleId` and no concept of a user-authored message body - its own table (`scheduled_messages`, `packages/database/prisma/migrations/20260815000000_phase21_6_scheduled_messages/`) and its own BullMQ queue (`scheduled-messages`). The actual outbound-send logic was extracted from `ConversationsController.sendMessage` into a shared `MessageSendService` (`apps/api/src/conversations/message-send.service.ts`) so a fired scheduled message goes through the identical connector/credential/persist/realtime/audit-log path a live reply does, not a parallel reimplementation that could drift from it - confirmed directly: a scheduled send that fails (no connected account, provider rejection, etc.) marks the row `failed` with a sanitized `lastError`, never silently drops it. New `GET /v1/scheduled-messages` (list, no pagination - same "small, user-owned list" reasoning as `GET /v1/connectors`) and `POST /v1/scheduled-messages/:id/cancel` (404 if already fired/cancelled/nonexistent, not a silent no-op). `apps/web/components/Inbox.tsx` gained a "Schedule" toggle next to the existing reply box (a `datetime-local` picker + "Confirm schedule" button) and an inline list of the current conversation's pending scheduled messages with a Cancel action, using new `apps/web/lib/api.ts` functions (`scheduleMessage`, `fetchScheduledMessages`, `cancelScheduledMessage`).
  ...
  **Disclosed gap, explicit**: unlike Phase 21.1/21.2, this phase's live-in-production verification does not include an actual real-provider delivery at the scheduled time - that requires an existing Telegram-linked conversation, a human sending a message first to seed it (same precondition `verify-phase21.1-production-live.mjs` has), and then waiting past the scheduled `sendAt` to confirm delivery, which was not run this session. What is verified: the scheduling/listing/cancellation API surface end to end, and the delayed-job-fires-and-updates-status mechanism locally with a mock (unconnected) conversation. Not marking the real-provider delivery leg as confirmed until it's actually run - same discipline as Phase 21.1's message-delivery claim.
