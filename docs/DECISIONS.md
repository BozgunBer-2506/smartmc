...
| [0014](adr/0014-custom-jwt-session-auth.md) | Custom JWT/Session Authentication Instead of Auth.js; Direct Token Issuance for First-Party Clients | Accepted | 2026-07-18 | Corrects `ARCHITECTURE.md`'s "Auth.js" mention - it has no NestJS integration and can't implement `DATABASE.md`'s `family_id` reuse-detection design. Authentication is custom Argon2id + `@nestjs/jwt` + hand-written session/rotation logic, implementing the already-documented behavior exactly. Phase 2 issues tokens directly (`POST /v1/auth/login`); the full OAuth2 Authorization Code + PKCE flow (`API.md` Section 7.1) remains the Phase 18 target for real external/marketplace clients. |
| [0015](adr/0015-rest-inbox-read-path-for-phase-3.md) | REST (Not GraphQL) for Phase 3's Inbox Read Path | Accepted | 2026-07-18 | `API.md` Section 10.3 frames the inbox read path as "primarily GraphQL" per ADR-0003, but no GraphQL server exists yet and standing one up would be new infrastructure Phase 3's scope explicitly rules out. `GET /v1/conversations`/`GET /v1/conversations/{id}/messages` ship as plain REST for now; GraphQL remains the Phase 9 (Smart Inbox) target once real client-shaped query variance justifies it. |
| [0016](adr/0016-interim-envelope-encrypted-secrets-store.md) | Interim Envelope-Encrypted Secrets Store for Connector Credentials | Accepted | 2026-07-19 | `SECURITY.md` requires an external secrets manager (Vault/AWS) for connector credentials, but none exists and provisioning one is disproportionate to Sprint 2's need. Ships a `SecretsService` interface backed by AES-256-GCM envelope encryption in a dedicated, non-soft-deletable Postgres table; swapping to a real secrets manager later changes one provider, not `LinkedAccount` or connector code. Disclosed as an interim, pre-production posture. |
| [0017](adr/0017-telegram-sync-and-reconciliation-strategy.md) | Telegram Sync and Reconciliation Strategy Given Bot API's Shape | Accepted | 2026-07-19 | Telegram's Bot API has no message-history endpoint and `getUpdates`/`setWebhook` are mutually exclusive, so `CONNECTOR_SDK.md` Section 4.3's "list endpoint diff" reconciliation and Section 8.1's bounded backfill can't be implemented as generically specified. Initial sync is a documented no-op (nothing to backfill); reconciliation uses `getWebhookInfo`'s backlog/error signal to trigger a real webhook-delete-drain-via-`getUpdates`-reregister cycle when needed. Telegram-specific, not a revision to the SDK's general contract. |
