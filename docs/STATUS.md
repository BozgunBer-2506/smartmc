...
12. **Phase 20.2 (Production Readiness - real cursor pagination) shipped and verified locally**, 2026-08-04 - see `ROADMAP.md`'s Phase 20 entry for the full technical writeup. All 11 list endpoints that needed it now use real Postgres keyset pagination behind one canonical `{data, pagination}` envelope, replacing hardcoded `take` caps and (for `GET /v1/rules`, `/v1/identity/merge-suggestions`, `/v1/conversations/:id/messages`) closing three fully-unbounded queries found during the migration audit. `scripts/verify-phase20.2-cursor-pagination.mjs` (14/14) walks a full multi-page cursor chain end to end. Not yet deployed to Railway/live-verified in production - next action.
...
2. **Phase 20 - Production Readiness**: 20.1 (real rate limiting) and 20.2 (real cursor pagination) are complete; 20.2 is verified locally (14/14, `verify-phase20.2-cursor-pagination.mjs`) but not yet deployed to Railway or live-verified in production - that's the immediate next action. 20.3 (`sortBy`/`order`), 20.4 (`ETag`/`If-Match`), and 20.5 (observability) remain, deliberately sequenced as independent sub-phases (each its own commit, live verification, and doc update) per explicit user direction rather than one large Phase 20 effort.
3. Otherwise, begin Phase 15 - Desktop (evaluate Tauri against what the Phase 14 PWA already delivers), or continue rounding out Phase 10-14's deferred scope (nested-condition-group UI, more trigger types, retry/circuit-breaker/DLQ, the simulator, per-user notification targeting, Waiting-On/Commitments, escalation rules, attachment ingestion, a real LLM provider, translation, semantic search, real brand icon art, workbox integration) per those phases' review docs' Future Work - whichever the user directs.
