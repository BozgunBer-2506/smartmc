...
2. **Phase 20 - Production Readiness**: 20.1 (real rate limiting) is complete and live-verified (see above). 20.2 (cursor pagination), 20.3 (`sortBy`/`order`), 20.4 (`ETag`/`If-Match`), and 20.5 (observability) remain, deliberately sequenced as independent sub-phases (each its own commit, live verification, and doc update) per explicit user direction rather than one large Phase 20 effort.
