...
| `ROADMAP.md` | 20 phases (Phase 19 - WhatsApp Connector added 2026-07-28), working rules, Phase 1-12 verified Definitions of Done |
| `STATUS.md` | This file |
...
Phase 12 (Search) is complete for its disclosed scope - real Postgres full-text search over messages, substring search over contacts, and the combined endpoint, verified end-to-end (9/9, `verify-phase12.mjs`). Before picking a next phase:

0. **Click-test the Rules, notification-preferences, and search UI in a real browser** (gaps #20/#22/#24 above) - create a rule, toggle enable/disable, run a test, view execution history, set silent hours and confirm the tiered chimes and suppression actually work live, and confirm the search box returns and opens the right results.
...
4. Otherwise, begin Phase 13 - AI, or continue rounding out Phase 10/11/12's deferred scope (nested-condition-group UI, more trigger types, retry/circuit-breaker/DLQ, the simulator, per-user notification targeting, Waiting-On/Commitments, escalation rules, attachment ingestion) per those phases' review docs' Future Work - whichever the user directs.
