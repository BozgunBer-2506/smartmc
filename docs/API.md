...
**Optimistic concurrency (item: optimistic concurrency)** - real, implemented as of `ROADMAP.md` Phase 20.4: resources with a `version` column expose it as an `ETag` response header (`GET /v1/rules/:id`, `GET/PATCH /v1/notification-preferences`); mutating requests must send `If-Match: "{version}"`. Its absence is `428 Precondition Required`, not a silent bypass. A mismatch is `412 Precondition Failed` (the standard HTTP status for a failed conditional request, RFC 7232 - corrected from an earlier draft of this section that said `409`) with `code: OPTIMISTIC_LOCK_FAILURE`, and the response carries a fresh `ETag` for the current server-side version so the client can retry without a second round trip. `GET` also honors `If-None-Match`, returning a bodyless `304 Not Modified` on a match. The check is atomic against what the client's `If-Match` actually declares - never against a version the server itself re-fetched moments earlier in the same request, which would only guard the microseconds between fetch and write, not the real "someone else edited this since I loaded the form" case conditional requests exist to catch. `NotificationPreference` didn't have a `version` column before this phase (or any conflict detection at all - `PATCH` was a blind upsert); a client that has never fetched it sends `If-Match: "new"` to create it for the first time. Scope: `Rule` and `NotificationPreference` are the only two resources with both a `version` column and an existing mutation endpoint today - `LinkedAccount` (connector settings) has no settings-mutation endpoint at all yet (only connect/disconnect), and `Workspace`/`Organization` settings (the single `aiEnabled` toggle) don't have a `version` column; both are deliberately deferred, not silently dropped - see `docs/STATUS.md`.
