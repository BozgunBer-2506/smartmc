...
import { defaultConnectorRegistry, SLACK_PROVIDER_KEY, type SlackConnector } from "@smc/connector-sdk";
...
/**
 * The periodic reconciliation pass CONNECTOR_SDK.md Section 4.3 requires
 * for Slack's "hybrid" ingestion mode - the same shape as
 * TelegramReconciliationService/DiscordReconciliationService, catching
 * anything a missed Events API delivery (Slack's own webhook, subject to
 * the same never-100%-reliable caveat as any provider push) would
 * otherwise silently drop. Slack's conversations.history endpoint makes
 * this a genuine list-and-diff pass, the same as Discord's.
 */
@Injectable()
export class SlackReconciliationService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(SlackReconciliationService.name);
  ...
  onModuleInit(): void {
    ...
    }, slackConfig.reconciliationIntervalMs());
  }
  ...
  async runOnce(): Promise<void> {
    ...
    const connector = defaultConnectorRegistry.get(SLACK_PROVIDER_KEY) as SlackConnector;

    const accounts = await prisma.linkedAccount.findMany({
      where: { status: { in: ["active", "degraded"] }, provider: { key: SLACK_PROVIDER_KEY } },
    });
    ...
    for (const account of accounts) {
      try {
        ...
        const result = await connector.reconcile(
          ...
          { credential: { botToken, teamId: account.externalAccountId }, linkedAccountId: account.id },
        );
        ...
        for (const message of result.messages) {
          ...
          const event = createEvent({
            ...
            producer: `connector-worker:${SLACK_PROVIDER_KEY}:reconciliation`,
            workspaceId: account.workspaceId,
            payload,
          });
        ...
        // Reconciliation walks channels one page per cycle (SlackConnector.syncChannels);
